...
| # | 内容 | 影响 |
|---|------|------|
| 1 | 7 个写入型技能顶部各加一行式「⚠ 写入硬约束」：`[[X]]` 目标必须已在 vault 文件名索引，不存在 → 降级 `**X**` 加粗，禁止写入死链（各技能差异化豁免：capture/daily 用户原文逐字保留、import 源库 wikilink 保留、project ADR 深路径按规范 §5 全路径） | capture / ingest / connect / daily / goal / project / import SKILL.md |
| 2 | query 顶部加「引用硬约束」：strict 模式结论挂的 `[[]]` 必须指向真实存在的笔记——空链接就是死链，宁可不写该句；自检清单同步强化 | query SKILL.md |
| 3 | 17 SKILL version → v1.10.3 | 全部 SKILL.md |

### 设计说明

- 防线分层不变：写入前拦截（本版前置强化）→ stats 死链统计（wikilink-conventions §8）→ manager repair_deadlinks 修复。本版只补「执行方漏读校验」缺口，不改判定口径。
- 一行式 vs init 的 6 条块：init 是唯一「全部产出物都错则全错」的根技能才值得 6 条块；写入技能的致命错误集中死链一条，一行足够且不稀释正文。

### 评测

capture split=test 两轮：**7/9 / 0.89**（A3.1=已记录 flaky + A5.3 网络抓取类单轮漂移）→ 复跑 **9/9 / soft 1.00**（满分恢复，方差确认——硬约束行对 capture 行为无回归）；flow split=train **0/4 / 0.73 → 1/4 / 0.84**（hard 恢复 + soft 上升，J 类振荡区间 0/4~2/4 内；四例 soft 0.57-0.83 且产物路径形态正确，失败为边界判定）。

---

## v1.10.2 · 2026-08-17 · init 硬约束前置（防部分阅读）

**摘要**：端到端安装测试（同日，见 v1.10.1 条目）暴露第二个独立根因——472 行 init SKILL.md 在单轮执行下被**部分阅读**（前段命中 → en 默认生效；中段截断 → 只铺基础 4 类模板；尾部未达 → 自检清单与步骤 5 新格式未执行）。本版把全部硬要求前置到文档最顶端，任何阅读深度下可见。

### 修复清单

| # | 内容 | 影响 |
|---|------|------|
| 1 | init SKILL.md 顶部新增 **「⚠ 执行硬约束」块**（6 条：28 模板实测清点 / schema+规范化指纹 / 根文件是且仅是 `_index.md` / `.kb-initialized` YAML 新格式 / config v1.10 语义 / 自检逐项核对后才可输出 ✓），并附部分阅读背景说明 | init SKILL.md |
| 2 | 步骤 5 示例 `skill_version: v1.8.0` 硬编码 → `{{实际当前技能版本}}` 占位 + 禁止照抄历史版本号注释（实测执行方照抄示例产出 v1.8.0 标记） | init SKILL.md |
| 3 | 17 SKILL version → v1.10.2 | 全部 SKILL.md |

### 同日环境侧处置（非仓库变更）

- 用户全局 `~/.claude/skills/` 与 `~/.agents/skills/` 的 17 技能已从 v0.1 升级到 v1.10.1（「vault 像老版本」第一风险源：多级安装目录新旧并存，用户级优先于项目级）
- 升级后端到端复测全过：28 模板 + 计数校验 / 指纹 `fa8ab113`（规范化）/ 根 `_index.md` / 自检断言全部生效

### 评测

capture split=test 两轮：**7/9 / 0.80**（G3 失败=已知 flaky「多余润色」+ A1.2 单轮漂移）→ 复跑 **8/9 / soft 0.96**（基线恢复，确认方差——本轮 capture SKILL 零内容变更）；flow split=train **2/4 / soft 0.73**（hard 持平，J 类区间 0.72~0.88）。

---

## v1.10.1 · 2026-08-17 · E:\knowledge 实证校准（指纹换行符规范化 + init 漂移断言）

**摘要**：用户以 v1.10.0 技能初始化 E:\knowledge 后报告「vault 像老版本」。实证核查推翻「资源链断裂 → 降级 stub」假设（① 级源命中，28 模板/schema 逐字节忠实复制），定位两个真实成因：① 指纹算法换行符敏感（分发链 CRLF/LF 字节漂移 → upgrade 比对误报）；② 执行漂移两例（根 `_readme.md` 旧先验 + config 旧注释模板），且既有自检断言未被执行方触发。

### 修复清单

| # | 内容 | 影响 |
|---|------|------|
| 1 | **指纹算法换行符规范化**：schema_sha256 与模板完整性校验均改为剔除 `\r` 后计算（换行符不敏感）；upgrade 比对兼容存量——旧算法指纹先原样比一次，未命中再按规范化指纹比 | init SKILL.md（步骤 5 字段表 / §3.7 upgrade 两行 / 模板铺设段） |
| 2 | **init 根文件名负向断言**：vault 根不得存在 `_readme.md`（根导航是且仅是 `_index.md`；`_readme.md` 只属于 `00_inbox/`）；发现即迁移后删除 | init SKILL.md 自检清单 |
| 3 | **init config 模板一致性断言**：`kb.config.yaml` 的 `language` 行注释须与 §3.1 当前模板一致（防执行方凭旧记忆写旧注释） | init SKILL.md 自检清单 |

### 校准备注

- **执行漂移第 5 类样本**（前 4 类：03/04 倒置、08/09/10 自创目录、config 模板、根文件名）——「自检清单存在但执行方不跑」是共性：正向清单约束力弱于硬校验（模板计数/指纹）。后续方向：把根文件名与 config 断言并入输出报告前的强制对照步骤。
- 外部 AI 分析「① 级源未命中 → 即兴 4 个降级 stub → v0.1 子集」不成立：安装副本即 v1.10.0 且自带资源，vault schema 与安装副本 diff 为空。**结论生成于未实测指纹与 diff 的推理**。

### 评测

capture split=test **8/9 hard / soft 0.96**（持平）；flow split=train **2/4 hard / soft 0.76**（hard 持平 v1.10.0，soft 在 J 类波动区间 0.72~0.88 内）。本轮仅 init 内容变更 + 17 技能版本号，无 capture/flow 路径改动。
