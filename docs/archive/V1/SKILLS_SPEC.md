...
2. **逐条处理**（可并行，由 research-agent 承担）：
   ...
   2. **分类去向**：concept → areas/、resource → resources/、decision → outputs/decisions/。
   3. **补全 frontmatter**：依据 DESIGN 第 6 节填全部字段。
   ...
   7. **生成 wikilinks**：扫描已存在笔记，找出标题/标签匹配项写入 `related`。
3. **冲突检测**：与已入库笔记相似度 >0.85，提示合并或区分。
4. **写入并反馈**：每条笔记的写入位置、类型、置信度、关联。
...
1. **调用 memory-agent**：基于情境做经验召回，找出历史上类似任务相关的 experience/pattern/decision。
...
3. **拉起子目录**：`notes/`（项目笔记）、`decisions/`（ADR）、`refs/`（参考资料 wikilink）。
4. **建立 _moc.md**：项目内索引页。
...
1. **状态检查**：未关闭的决策/笔记提醒。
2. **迁移**：`projects/<slug>/` → `archive/projects/<slug>/`。
3. **归档复盘**：生成项目复盘草稿。
4. **清理引用**：更新指向该项目的 wikilinks。
