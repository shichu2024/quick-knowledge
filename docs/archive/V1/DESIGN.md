...
- **知识密度** —— 每条笔记的信噪比。靠原子化笔记、结构化 frontmatter、Ingest 阶段去重来保证。
- **调用频次** —— 笔记被回看和引用的次数。靠 MOC、双链、Query 引用机制来放大。
- **验证深度** —— 笔记里结论的可信度。靠 `confidence` 字段、`source` 引用、周期 Review 来积累。

三个变量任一归零，知识库价值就归零。所有技能的设计目标都是同时抬升这三条曲线。
...
| 关联提醒 | 主动提示「你之前在 X 项目遇到过类似问题：[[链接]]」 |
| 防重复犯错 | 检测当前计划是否与某条 `experience` 教训冲突，提出警告 |
| 信念核对 | 决策前检索相关的 `belief`/`principle`，提示是否一致 |

**触发**：`quick-kb-advisor`、`quick-kb-project`（新开项目时）、`quick-kb-goal` 内部调用，或用户直接唤起（「我之前是不是做过类似的东西？」）。
